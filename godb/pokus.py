...
def raw_feature_extractors(p_type, all_pat, bw_game_summarize):
    # Process One Side List
    if p_type == 'all_features':
    ...
    if p_type == 'fast_strength':
        return [
            (BWPatternVectorGenerator( bw_game_summarize,
                                       PatternVectorMaker(all_pat, 400) ), VectorArithmeticMeanMerger()),
            (BWLocalSeqVectorGenerator(10), VectorArithmeticMeanMerger()), 
            (BWBdistVectorGenerator(by_moves=[10, 64, 200]), VectorArithmeticMeanMerger()), 
            (BWCaptureVectorGenerator(by_moves=[60, 240]), VectorArithmeticMeanMerger()),
    ...
    raise ValueError("Unknown p_type '%s'"%p_type)

def get_game_summarizer(spatial_dict):
    return partial_right(process_game,
...
def get_feature_extractors_from_file(p_type, all_pat, spatial_dict):
    bw_game_summarize = get_game_summarizer(spatial_dict)
    return raw_feature_extractors(p_type, all_pat, bw_game_summarize)

def get_feature_extractors(p_type, SPATIAL_dict_data, ALL_PAT_OSL, t):
    with t(log=True):
...
def main():
    ...
    fes = get_feature_extractors(p_type, SPATIAL_dict_data, ALL_PAT_OSL, t)
