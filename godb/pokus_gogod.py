...
def get_feature_extractors(SPATIAL_dict_data, ALL_PAT_OSL, t=timer.Timer()):
    ...
    """
    # vector generators ; game -> BlackWhite vector
    vg_pat = BWPatternVectorGenerator( bw_game_summarize, PatternVectorMaker(all_pat, N) )
    
    gen_n_merge2 = [
        (vg_pat, VectorApply(VectorSumMerger(), finish_fc=natural_rescale)),
        (vg_pat, VectorArithmeticMeanMerger()), 
        (BWLocalSeqVectorGenerator(8), VectorArithmeticMeanMerger()), 
        (BWLocalSeqVectorGenerator(14), VectorArithmeticMeanMerger()), 
        (BWBdistVectorGenerator(by_moves=[10, 54, 208]), VectorArithmeticMeanMerger()), 
        (BWCaptureVectorGenerator(by_moves=[60, 220]), VectorArithmeticMeanMerger()),
    ...
    best = [
        (BWPatternVectorGenerator( bw_game_summarize,
                                   PatternVectorMaker(all_pat, 600) ), VectorArithmeticMeanMerger()),
        ...
        (vg_win_stat, VectorArithmeticMeanMerger()),
        (vg_points_stat, VectorArithmeticMeanMerger())
    ]
...
def main():
    ...
    with t(log=True):
        ...
        if choice ==  'KGS_wide_120':
            ...
            assert config.OUTPUT_DIR =='/media/VFS/OUT_KGS_wide'
